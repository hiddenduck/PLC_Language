    ...
    if coluna[1] == '':
        ...
        if coluna[2] == '':
            if coluna[3] == '':
                ...
                fun_list.append((eval(coluna[3]),fun_pos,coluna[1],''))
                patern += r'([^{,]+),'    
                replace += r'%s:\%d\n' % (coluna[0],i)
...
for linha in input:
    ...
    for fun,pos,lower,upper in fun_list:
        if upper == '':
            linha = re.sub(r'((?:[^,]+,){%d})([^,]+(?:,[^,]+){%d}),{0,%d}'%(pos,(int(lower)-1),int(lower)),lambda x: x.group(1) + str(fun(map(int,re.split(r',',x.group(2)))))+r',',linha)
        else:
            linha = re.sub(r'((?:[^,]+,){%d})([^,]+(?:,[^,]+){%d,%d}),{0,%d}'%(pos,(int(lower)-1),(int(upper)-1),int(upper)),lambda x: x.group(1) + str(fun(map(int,re.split(r',',x.group(2)))))+r',' ,linha)
